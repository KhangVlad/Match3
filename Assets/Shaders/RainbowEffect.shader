Shader "Custom/RainbowGradient"
{
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            //     float rad = (atan2(IN.texcoord.y - 0.5, IN.texcoord.x - 0.5) + (_DegOFfset / 360 * 6.283184) + time) % 6.283184 / 6.283184;
            //     // radial gradient
            //     color.xyz *= HUEtoRGB(rad);
            //     // vertical gradient
            //     //color.xyz = HUEtoRGB(IN.texcoord.y);
            //     // ripple effect
            //     color.xyz += float3(1, 1, 1) * saturate(-abs(abs(distance(float2(0.5, 0.5), IN.texcoord) - time) % _FlashInterval - _FlashLength) + _FlashLength) * _FlashDensity;
            //     // hue testing
